...
# def feature_dropping(data, drop_prob=0.1):
#     l2_norms = torch.norm(data, p=2, dim=(1, 2, 3))
    
#     # 根据L2范数大小选择丢弃的特征
#     mask = l2_norms < l2_norms.mean() + drop_prob * l2_norms.std()
#     mask = mask.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
#     return data * mask

def feature_dropping(data, drop_prob=0.1):
    # 生成一个与data形状相同的随机掩码
    mask = torch.rand_like(data) > drop_prob
    return data * mask


@iterative_unlearn
def FD(data_loaders, model, criterion, optimizer, epoch, args, mask=None):
    ...
    if args.dataset == "cifar100" or args.dataset == "TinyImagenet":
        ...
        for it, (image, target) in enumerate(train_loader):
            ...
            # 应用特征丢弃
            image = feature_dropping(image, drop_prob=0.1)
        ...
        for i, (image, target) in enumerate(forget_loader):
            ...
            # 应用特征丢弃
            image = feature_dropping(image, drop_prob=0.3)
